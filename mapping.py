...
def init_transform_matrix():
    ...
    infrared_xmls = [osp.join('E:\CVP\duiqicode\KJW-MAP', 'DJI_{:0>4}_R.xml'.format(id-1)) for id in ids]
...
def processing_data(H, image_pair, crop_ratio = 0.2, progress=False):
   ...
   # warp rgb image to infrared image
    ...
    # cv2.imwrite('DJI_000j.jpg', wrap)
    cv2.imshow('infrared', infrared_img)
    # cv2.imwrite('DJI_000i_R.jpg', infrared_img)

    for i in range(1,):
        cv2.imwrite('i', wrap)
    for j in range(2,):
        cv2.imwrite('j', infrared_img)
...
if __name__ == '__main__':
    ...
    for imagename in imagenames:
        if 'R' in imagename:
            idstr = imagename[imagename.find('_')+1:imagename.rfind('_')]
            id = int(idstr)
            src_infrared_path = osp.join(root_dir, imagename)
            src_rgb_path = osp.join(root_dir, 'DJI_{:0>4}.jpg'.format(id+1))
            processing_data(H, [src_rgb_path, src_infrared_path])
